...
async function getBlogPost(slug: string) {
  const { data, error } = await supabase.from('blog_posts').select('*').eq('slug', slug).eq('status', 'published').eq('lang', 'de').single();
...
async function getRelatedPosts(category: string, currentSlug: string) {
  const { data } = await supabase.from('blog_posts').select('slug, title, category, featured_image').eq('status', 'published').eq('lang', 'de').eq('category', category).neq('slug', currentSlug).limit(2);
  return data || [];
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params;
  const { data } = await supabase.from('blog_posts').select('title, excerpt').eq('slug', slug).eq('status', 'published').eq('lang', 'de').single();
    ...
    <article className="section">
      <div className="container-narrow">
        <nav className="mb-8 flex items-center gap-2 text-sm text-gray-500">
          <Link href="/de" className="hover:text-accent">{t.common.home}</Link><span>/</span>
          <Link href="/de/blog" className="hover:text-accent">{t.blog.title}</Link><span>/</span>
      ...
      <div className="container-narrow">
        ...
        <div className="mt-16 p-10 bg-gray-50 border-l-4 border-accent">
          ...
          <Link href="/de/anfrage" className="btn-minimal-lg">{t.blog.talkToExpert}</Link>
        </div>
          ...
          <div className="mt-20 pt-12 border-t border-gray-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {relatedPosts.map((rp: any) => (
                <Link key={rp.slug} href={`/de/blog/${rp.slug}`} className="group">
