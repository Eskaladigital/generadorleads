...
const staticRoutes = [
  ...
  { key: 'destinations', desc: 'Guia de destinos' },
  { key: 'services', desc: 'Serviços para estrangeiros' },
  { key: 'blog', desc: 'Artigos e guias' },
  { key: 'professionals', desc: 'Diretório de profissionais' },
  { key: 'about', desc: 'Sobre nós' },
  { key: 'contact', desc: 'Formulário de contacto' },
  { key: 'quote', desc: 'Solicitar orçamento' },
  { key: 'privacy', desc: 'Proteção de dados' },
  { key: 'terms', desc: 'Termos de utilização' },
  { key: 'cookies', desc: 'Utilização de cookies' },
];
...
export default async function MapaDoSitePage() {
  ...
  const allLandingPages = landingPages.map((landing: { slug: string; servicio_nombre: string; ciudad_nombre: string }) => ({
    ...
    titulo: `${landing.servicio_nombre} em ${landing.ciudad_nombre}`,
  }));
        ...
        <div className="container-base">
          <div className="max-w-3xl">
            ...
            <p className="text-lg text-white/90">
              Navegue por todas as páginas disponíveis na Health4Spain. Encontre destinos, serviços, artigos do blog e mais.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="py-10 md:py-14">
        <div className="container-base">
          <div className="max-w-4xl mx-auto space-y-12">
            ...
            <div>
              <h2 className="font-heading text-2xl font-bold text-gray-900 mb-4 pb-2 border-b border-gray-200">
                Destinos ({allDestinos.length} cidades)
              </h2>
            ...
            <div>
              <h2 className="font-heading text-2xl font-bold text-gray-900 mb-4 pb-2 border-b border-gray-200">
                Serviços ({allServicios.length} serviços)
              </h2>
            ...
            {allLandingPages.length > 0 && (
              <div>
                <h2 className="font-heading text-2xl font-bold text-gray-900 mb-4 pb-2 border-b border-gray-200">
                  Páginas Serviço × Cidade ({allLandingPages.length} páginas)
                </h2>
                <p className="text-sm text-gray-600 mb-4">Páginas especializadas por serviço e localização</p>
            ...
            <div>
              <h2 className="font-heading text-2xl font-bold text-gray-900 mb-4 pb-2 border-b border-gray-200">
                Artigos do Blog ({blogPosts.length} artigos)
              </h2>
              {blogPosts.length > 0 ? (
                ...
                <p className="text-gray-600 text-sm">Ainda não há artigos publicados.</p>
              )}
            </div>

            <div className="bg-gray-50 rounded-lg p-6 mt-8">
              <h3 className="font-semibold text-gray-900 mb-2">Informações do Mapa do Site</h3>
              <ul className="text-sm text-gray-600 space-y-1">
                ...
                <li>• Última atualização: {new Date().toLocaleDateString('pt-PT', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</li>
                <li>• Os artigos do blog são atualizados automaticamente</li>
              </ul>
            </div>
          </div>
        </div>
