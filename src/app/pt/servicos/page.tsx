    ...
    <>
      ...
      <section className="section-alt">
        <div className="container-narrow">
          <Breadcrumbs items={[{ label: t.common.breadcrumbHome, href: '/pt' }, { label: t.servicesPage.title }]} />
          <ul className="service-list-minimal">
            {servicios.map((servicio, index) => {
              const numero = String(index + 1).padStart(2, '0');
              return (
                <li key={servicio.slug}>
                  <Link href={`/pt/solicitar?servicio=${servicio.slug}`} className="service-item-minimal block w-full hover:bg-white hover:pl-4 transition-all group">
