...
const staticRoutes = [
  { key: 'home', desc: 'Main page' },
  { key: 'destinations', desc: 'Destinations guide' },
  { key: 'services', desc: 'Services for foreigners' },
  { key: 'blog', desc: 'Articles and guides' },
  { key: 'professionals', desc: 'Professional directory' },
  { key: 'about', desc: 'About us' },
  { key: 'contact', desc: 'Contact form' },
  { key: 'quote', desc: 'Request quote' },
  { key: 'privacy', desc: 'Data protection' },
  { key: 'terms', desc: 'Terms of use' },
  { key: 'cookies', desc: 'Cookie use' },
];
        ...
        <div className="container-base">
          <div className="max-w-3xl">
            ...
            <p className="text-lg text-white/90">
              Browse all available pages on Health4Spain. Find destinations, services, blog articles and more.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="py-10 md:py-14">
        <div className="container-base">
          <div className="max-w-4xl mx-auto space-y-12">
            ...
            <div>
              <h2 className="font-heading text-2xl font-bold text-gray-900 mb-4 pb-2 border-b border-gray-200">
                Destinations ({allDestinos.length} cities)
              </h2>
            ...
            {allLandingPages.length > 0 && (
              <div>
                <h2 className="font-heading text-2xl font-bold text-gray-900 mb-4 pb-2 border-b border-gray-200">
                  Service × City Pages ({allLandingPages.length} pages)
                </h2>
                <p className="text-sm text-gray-600 mb-4">Specialised pages by service and location</p>
            ...
            <div>
              <h2 className="font-heading text-2xl font-bold text-gray-900 mb-4 pb-2 border-b border-gray-200">
                Blog Articles ({blogPosts.length} articles)
              </h2>
              {blogPosts.length > 0 ? (
                ...
                <p className="text-gray-600 text-sm">No articles published yet.</p>
              )}
            </div>

            <div className="bg-gray-50 rounded-lg p-6 mt-8">
              <h3 className="font-semibold text-gray-900 mb-2">Sitemap Information</h3>
              <ul className="text-sm text-gray-600 space-y-1">
                <li>• Total pages: {rutasEstaticas.length + allDestinos.length + allServicios.length + allLandingPages.length + blogPosts.length}</li>
                <li>• Last updated: {new Date().toLocaleDateString('en-GB', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</li>
                <li>• Blog articles are updated automatically</li>
              </ul>
            </div>
          </div>
        </div>
