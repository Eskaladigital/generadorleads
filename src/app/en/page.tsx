    ...
    <>
      ...
      <section className="section">
        ...
        <div className="container-narrow">
          <div className="service-grid-2x2">
            {SERVICIOS.map((s) => (
              <Link key={s.id} href={`/en/request?servicio=${s.id}`} className="service-card">
      ...
      <section className="section-blue-light">
        ...
        <div className="container-narrow">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {AUDIENCIAS.map((a) => (
              <Link key={a.id} href={`/en/request?perfil=${a.id}`} className="profile-card">
      ...
      <section className="section">
          ...
          <div>
            ...
            <div className="destinos-lista">
              {CIUDADES_MURCIA.map((c) => (
                <Link key={c.slug} href={`/en/request?ciudad=${c.slug}`}>{c.nombre} <span>{t.home.request} →</span></Link>
              ))}
            </div>
          </div>
          <div>
        ...
        <div className="text-center mt-12">
          <Link href="/en/destinations" className="text-lg font-semibold text-black border-b-2 border-accent pb-1 hover:border-b-4 transition-all inline-block">{t.home.viewAllCities} →</Link>
        </div>
      </section>
        ...
        <div className="container-base">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-16 items-center">
            <div>
              ...
              <Link href="/en/request" className="btn-minimal-white">{t.home.requestContactNow}</Link>
            </div>
