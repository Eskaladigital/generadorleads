...
const staticRoutes = [
  { key: 'home', desc: 'Page principale' },
  { key: 'destinations', desc: 'Guide des destinations' },
  { key: 'services', desc: 'Services pour étrangers' },
  { key: 'blog', desc: 'Articles et guides' },
  { key: 'professionals', desc: 'Répertoire des professionnels' },
  { key: 'about', desc: 'À propos' },
  { key: 'contact', desc: 'Formulaire de contact' },
  { key: 'quote', desc: 'Demander un devis' },
  { key: 'privacy', desc: 'Protection des données' },
  { key: 'terms', desc: "Conditions d'utilisation" },
  { key: 'cookies', desc: 'Utilisation des cookies' },
];
...
export default async function PlanDuSitePage() {
  ...
  const allLandingPages = landingPages.map((landing: { slug: string; servicio_nombre: string; ciudad_nombre: string }) => ({
    ...
    titulo: `${landing.servicio_nombre} à ${landing.ciudad_nombre}`,
  }));
        ...
        <div className="container-base">
          <div className="max-w-3xl">
            ...
            <p className="text-lg text-white/90">
              Parcourez toutes les pages disponibles sur Health4Spain. Trouvez destinations, services, articles du blog et plus.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="py-10 md:py-14">
        <div className="container-base">
          <div className="max-w-4xl mx-auto space-y-12">
            ...
            <div>
              <h2 className="font-heading text-2xl font-bold text-gray-900 mb-4 pb-2 border-b border-gray-200">
                Destinations ({allDestinos.length} villes)
              </h2>
            ...
            <div>
              <h2 className="font-heading text-2xl font-bold text-gray-900 mb-4 pb-2 border-b border-gray-200">
                Services ({allServicios.length} services)
              </h2>
            ...
            {allLandingPages.length > 0 && (
              <div>
                <h2 className="font-heading text-2xl font-bold text-gray-900 mb-4 pb-2 border-b border-gray-200">
                  Pages Service × Ville ({allLandingPages.length} pages)
                </h2>
                <p className="text-sm text-gray-600 mb-4">Pages spécialisées par service et localisation</p>
            ...
            <div>
              <h2 className="font-heading text-2xl font-bold text-gray-900 mb-4 pb-2 border-b border-gray-200">
                Articles du Blog ({blogPosts.length} articles)
              </h2>
              {blogPosts.length > 0 ? (
                ...
                <p className="text-gray-600 text-sm">Aucun article publié pour le moment.</p>
              )}
            </div>

            <div className="bg-gray-50 rounded-lg p-6 mt-8">
              <h3 className="font-semibold text-gray-900 mb-2">Informations du Plan du Site</h3>
              <ul className="text-sm text-gray-600 space-y-1">
                <li>• Total des pages : {rutasEstaticas.length + allDestinos.length + allServicios.length + allLandingPages.length + blogPosts.length}</li>
                <li>• Dernière mise à jour : {new Date().toLocaleDateString('fr-FR', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</li>
                <li>• Les articles du blog sont mis à jour automatiquement</li>
              </ul>
            </div>
          </div>
        </div>
